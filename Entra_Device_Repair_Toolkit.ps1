...
param(
 ...
 [switch]$RunRegistrationDiagnostics,
 [switch]$DryRun,[switch]$Yes,
 [string]$OutputPath=(Join-Path $env:ProgramData 'EntraDeviceRepair')
)
...
if(-not($RefreshPrt -or $TriggerDeviceJoin -or $RestartIdentityServices -or $RunRegistrationDiagnostics)){Write-Error 'Choose at least one repair action.';exit 2}
...
State $before
if(-not $Yes -and -not $DryRun){if((Read-Host 'Apply selected Entra device repairs? Type YES') -ne 'YES'){Log 'Cancelled.';exit 10}}
if($RestartIdentityServices){foreach($s in 'TokenBroker','wlidsvc','NgcCtnrSvc','dmwappushservice'){if(Get-Service $s -ErrorAction SilentlyContinue){Act "Restarting service $s" {Restart-Service $s -Force -ErrorAction Stop}}}}
if($RefreshPrt){Act 'Requesting Primary Refresh Token renewal' {& dsregcmd.exe /refreshprt|Out-File (Join-Path $run 'refresh-prt.txt');if($LASTEXITCODE -ne 0){throw "dsregcmd exited $LASTEXITCODE"}}}
...
if($RunRegistrationDiagnostics){Act 'Collecting detailed registration diagnostics' {& dsregcmd.exe /status /debug|Out-File (Join-Path $run 'dsregcmd-debug.txt')}}
